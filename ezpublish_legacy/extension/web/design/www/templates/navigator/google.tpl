...
<ul>
     ...
     {switch match=$:item_previous|lt(0) }
       {case match=0}<li class="prev"><a href={concat($page_uri,$:item_previous|gt(0)|choose('',concat($:offset_text,$:item_previous)),$:view_parameter_text,$page_uri_suffix)|ezurl}>← {"Previous"|i18n("design/standard/navigator")}</a></li>{/case}
       {case match=1}
       {/case}
     {/switch}
...
{if $:current_page|gt($:left_max)}<li><a href={concat($page_uri,$:view_parameter_text,$page_uri_suffix)|ezurl}>1</a></li>
    {if sub($:current_page,$:left_length)|gt(1)}<li><a href="#">...</a></li>
{/if}
{/if}
    ...
    <li class="active"><a href={concat($page_uri,$:page_offset|gt(0)|choose('',concat($:offset_text,mul($:page_offset,$item_limit))),$ViewParameter:view_parameter_text,$page_uri_suffix)|ezurl}>{$:current_page|inc}</a></li>
