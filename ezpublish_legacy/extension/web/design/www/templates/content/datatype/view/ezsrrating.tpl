...
<div class="totals">
{'%current_rating/5'|i18n('extension/ezstarrating/datatype', '', hash( '%current_rating', concat('<span id="ezsr_average_', $attribute.id, '" class="ezsr-average-rating">', $rating.rating_average|wash|mul(10)|round()|div(10), '</span>') ))}
({'%rating_count'|i18n('extension/ezstarrating/datatype', '', hash( '%rating_count', concat('<span id="ezsr_total_', $attribute.id, '">', $rating.rating_count|wash, '</span>') ))} voto{if $rating.rating_count|ne(1)}s{/if})
 {if $attribute.data_int} {'disabled'|i18n('extension/ezstarrating/datatype')}.{/if}
</div>
<p id="ezsr_just_rated_{$attribute.id}" class="ezsr-just-rated hide">{'¡Gracias por tu voto!'|i18n('extension/ezstarrating/datatype', 'When rating')}</p>
